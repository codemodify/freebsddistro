...
void TwoDRenderer::loadMap( QString filePath )
{
	...
	while( false == node.isNull() )
	{
		...
		_myBalls.insert( ballID, reinterpret_cast<MeBall*>(gameObject) );
		ballID++;
	}
	...
	while( false == node.isNull() )
	{
		...
		_enemyBalls.insert( ballID, reinterpret_cast<EnemyBall*>(gameObject) );
		ballID++;
	}
...
void TwoDRenderer::playMouseWasReleased( QMouseEvent* event )
{
	...
	GameObject* gameObject = reinterpret_cast<GameObject*>( item );
	if( 0 == gameObject )
		return;
	
	if( (false==_myBalls.values().contains(reinterpret_cast<MeBall*>(gameObject))) && (false==_enemyBalls.values().contains(reinterpret_cast<EnemyBall*>(gameObject))) )
		return;
	
	Move move;
	move._ballID = _myBalls.values().contains(reinterpret_cast<MeBall*>(gameObject)) ? _myBalls.keys(reinterpret_cast<MeBall*>(gameObject)).at(0) : _enemyBalls.keys(reinterpret_cast<EnemyBall*>(gameObject)).at(0);
	move._angle = 0;
	move._power = 10;
...
void TwoDRenderer::timerEvent( QTimerEvent* event )
{
	...
	foreach( Move* move, _moves )
	{
		if( true == _myBalls.contains(move->_ballID) )
		{
			...
			_moves.removeAll( move );			
			delete move;
		}
		else if( true == _enemyBalls.contains(move->_ballID) )
		{
			EnemyBall* ball = _enemyBalls[move->_ballID];
			ball->moveBy( -1,0 );
