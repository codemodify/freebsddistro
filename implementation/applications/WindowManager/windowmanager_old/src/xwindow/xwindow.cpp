	...
	_isMaximized(false)
{
	...
	// getting the windw's geometry, these numbers will be taken as 
	// start point for positioning different GUI controls
	...
	XSelectInput( XLibWrapper::getDisplay(), _window, EVENT_MASK_FOR_NORMAL_WINDOW );
	...
	// apply different properties related to the control we have
	applyProperties();
...
{
	std::string text = XLibWrapper::getWindowTitleText( _window );
...
void XWindow::applyProperties()
{
	...
	//XLibWrapper::moveWindow( _window, _x, _y+TITLE_SIZE );
