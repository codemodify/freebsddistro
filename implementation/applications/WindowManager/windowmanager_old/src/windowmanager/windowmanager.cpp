...
void WindowManager::setRootWindowsMask()
{
	for( int index=0; index< _rootWindows.size(); ++index )
		XSelectInput( _x11Display, _rootWindows.at(index), EVENT_MASK_FOR_ROOT_WINDOW );
}
...
bool WindowManager::initEnvironment()
{
	...
	// inform X11 server that for us is important to not use its buffering feature, all the action must happen immidiately
	XSynchronize( _x11Display, True );
	...
	for( int screenIndex = 0; screenIndex < screensCount; ++screenIndex )
	{
		...
		if( BadWindow == XSelectInput(_x11Display,rootWindow,EVENT_MASK_FOR_ROOT_WINDOW) )
			return false;
	}
	...
	_taskBar->show();
	//XLibWrapper::showWindow( _taskBar->winId() );
...
void WindowManager::handleConfigureRequest( XConfigureRequestEvent* event )
{
	...
	// Or better be kinder and help it out as usually non RM governments do, probably this will make him stay and leave the country in search for a better life
...
void WindowManager::handleMapRequest( XMapRequestEvent* event )
{
	...
	if( true == XLibWrapper::isWindowSupposedTohaveDecorations(window) )
	{
		...
		// in all other cases it must be a shaped or popup window - we don't care about that, we just show it naked, let people look at it how ugly it is
...
void WindowManager::handleDestroyNotification( XDestroyWindowEvent* event )
{
	...
	// the devil will be with you - better pay if you want A god. And don't forget to kiss my hand and call me father - because I've learned for 
	// that and my marks from the speciffic schools allow me to be called father.
