...
QTextOStream qapp::logmsg(stderr);
...
#ifdef DEBUGMSG
	qapp::logmsg << "class xwindow constructed (WId:" << winId() << ")\n";
#endif	
}
...
void xwindow::resize_client(int px, int py, int pw, int ph)  // dimensions include borders
{
	...
	{
#ifdef DEBUGMSG
		qapp::logmsg << "resize child window (WId:" << clientid << ")\n";
#endif
...
#ifdef DEBUGMSG
	qapp::logmsg << "changed to withdrawn (WId:" << winId() << ")\n";
#endif
}
...
{
#ifdef DEBUGMSG
	qapp::logmsg << "change active client to (WId:" << winId() << ")\n";
#endif
...
int xwindow::get_clientstate(void)
{
	...
	{
		qapp::logmsg << "WM: cannot get wm_state for " << clientid << '\n';
		return(None);
	}	
...
void xwindow::get_colormaps(void)
{
	...
	if(nm > 1000)
	{
		qapp::logmsg << "More than 1000 colormaps for " << clientname << " - property rejected\n";
		return;
	}	
...
#ifdef DEBUGMSG
	qapp::logmsg << "client has WM_COLORMAP_WINDOWS property\n";
#endif	
...
#ifdef DEBUGMSG
	qapp::logmsg << "res_name set to " << res_name << '\n';
#endif
...
#ifdef DEBUGMSG
	qapp::logmsg << "rebuilding window frame (WId:" << winId() << ")\n";
#endif	
...
#ifdef DEBUGMSG
	qapp::logmsg << "reading WMNormalHints (WId:" << winId() << ")\n";
#endif	
...
void xwindow::get_wmhints(void)  // get WMHints
{
	...
	{
#ifdef DEBUGMSG
	qapp::logmsg << "reading WMHints (WId:" << winId() << ")\n";
#endif	
...
#ifdef DEBUGMSG
	qapp::logmsg << "WM_NAME set to " << wmname << '\n';
#endif
...
#ifdef DEBUGMSG
	qapp::logmsg << "class xwindow destructed (WId:" << winId() << ")\n";
#endif	
}
