...
void apbar::paletteChange(const QPalette &)  // propagate property event to clients
{
	...
	for(int i=0; i < ap.size(); i++)
	{
		...
		//xe.atom = WindowManager::wm_resource_manager;
...
{/*
	...
	{
		cur = ap.at(i);
		if(WindowManager::apclients[cur->name] == 0)
		{
			XGetWindowProperty(QX11Info::display(), cur->w, WindowManager::kde_net_wm_system_tray_window_for, 0, 1L, FALSE, XA_WINDOW, &type, &format, &nitems, &extra, &data);
