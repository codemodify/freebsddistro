...
void xwindow::t_maximize(void)
{
	...
	if(maxstate != 1)  // maximize
	{
		...
		if(defaults::tmx1 == -1 || tstate || ((pflags & WindowManager::NoTile) && WindowManager::is_tileddesk()))   // not user configured
		{
			cw = dt->width();
...
void xwindow::s_maximize(void)
{
	...
	if(tstate)
	{
		WindowManager::tile_maximize(this);
		return;
	}
	...
	if(maxstate != 2)  // maximize
	{
		...
		if(defaults::smx1 == -1 || ((pflags & WindowManager::NoTile) && WindowManager::is_tileddesk()))
		{
			cw = dt->width();
			ch = dt->height();
			getsize(&cw, &ch);
...
void xwindow::release_move(QMouseEvent *event)
{
	...
	if(tstate && ! WindowManager::is_curdesk(this))
		tstate = FALSE;
}
...
{
	WindowManager::winf->show_info(this, wmname, clientfqdn, command, res_name, res_class, inc_w, inc_h, base_w, base_h);
}
...
{
	if((pflags & (WindowManager::Sticky|WindowManager::NoTile)) || transfor != None)
		return FALSE;
...
void xwindow::minimize_frame(bool mf)
{
	...
	if(shaped || (pflags & WindowManager::NoResize))
		borderh -= defaults::lowerborderheight;
...
void xwindow::setinactive(void)
{
	...
	if(actpal || urgpal)
	{
		setPalette(*WindowManager::ipal);  // set inactive colors
		stopautofocus();
		actpal = FALSE;
	}
...
void xwindow::seturgent(void)
{
	if(! urgpal)
	{
		setPalette(*WindowManager::upal);
		tb_pb->change_palette(*WindowManager::upal, this);
		urgpal = TRUE;
	}
}
...
void xwindow::raise()
{
	if(mrb != NULL || WindowManager::winf->get_client() != NULL)
		return;
...
void xwindow::map_normal(void)
{
	if(! isVisible())
	{
		...
		if(is_tileable() && WindowManager::is_tileddesk()) 
		{
			if(clients.indexOf(this) == -1)
				return;

			WindowManager::tile_order(trsize?this:WindowManager::tmaxclient);
		}	
...
void xwindow::unmap(void)
{
	...
	if(tstate && WindowManager::is_curdesk(this))
	{
		if(WindowManager::tmaxclient == this)
			WindowManager::tmaxclient = NULL;
...
{
	if(WindowManager::tmaxclient == this && WindowManager::is_tileddesk())
		trsize = TRUE;
...
void xwindow::focus_mouse(bool wlist)  // map and set mouse (generates enter event -> focus)
{
	...
	if(WindowManager::smode)  // screen mode
	{
		if(! isVisible())
			return;
	...
	if(mid > dt->width())
	{
		tb_pg->change_desk(WindowManager::adesk+(x()/(dt->width()-1)));
	}	
	else if(mid < 0)
	{
		tb_pg->change_desk(WindowManager::adesk+(x()/(dt->width()+1))-1);
	}
...
void xwindow::setchildfocus(long timestamp) // set to active and focus to child
{
	...
	if(prot_takefocus)  // WM_TAKE_FOCUS protocol
		send_wmprotocol(WindowManager::wm_take_focus, timestamp);
}
...
void xwindow::getwmprotocols(void)
{
	...
	if(XGetWMProtocols(QX11Info::display(), clientid, &protocols, &nprot))
	{
		for(i=0; i < nprot; i++)
		{
			if(protocols[i] == WindowManager::wm_delete)
			{
				prot_delete = TRUE;
			}	
			else if(protocols[i] == WindowManager::wm_take_focus)
			{
				prot_takefocus = TRUE;
			}
		}
...
void xwindow::send_wmprotocol(long data0, long data1)  // send protocol message to child window
{
	...
	event.xclient.message_type = WindowManager::wm_protocols;
...
int xwindow::get_clientstate(void)
{
	...
	if(XGetWindowProperty(QX11Info::display(), clientid, WindowManager::wm_state, 0L, 1L, FALSE, WindowManager::wm_state, &ret_type, &format, &n, &extra, (uchar **)&data) != Success || n == 0)
	{
...
void xwindow::set_clientstate(int state)
{
	...
	XChangeProperty(QX11Info::display(), clientid, WindowManager::wm_state, WindowManager::wm_state, 32, PropModeReplace, (uchar *)data, 2);
}
...
void xwindow::wdestroy(void)  // destroy client
{
	...
	{
		send_wmprotocol(WindowManager::wm_delete, CurrentTime);
#ifdef DEBUGMSG
		logmsg << "soft kill\n";
#endif
	}
...
void xwindow::setcmapfocus(void)
{
	...
	for(int i = ncmaps-1; i >= 0; i--)
	{
		...
		WindowManager::install_colormap(attr.colormap);
	}	
    	if(! installed)
   		WindowManager::install_colormap(cmap);
}
...
int xwindow::get_clientflags(void)
{
	...
	cclname += ',';
	cclname += res_class;
	int clf = WindowManager::cprops[cclname];

	if(clf)
		return(clf);

	if((clf = WindowManager::cprops[clientname]))
		return(clf|WindowManager::ClassOnly);
...
void xwindow::set_pflags(int tflags)
{
	...
	if(pflags & (WindowManager::NoTile|WindowManager::Sticky))
		unset_tile();

	int mask = WindowManager::SmallFrame|WindowManager::NoResize;
	...
	if(! (oldflags & WindowManager::Sticky) && (flags & WindowManager::Sticky))  // move to current desk
	{
		int dx = x()/dt->width();
...
{
	if((pflags & WindowManager::SmallFrame) || (uborderh == defaults::lowerborderheight))
		return;
	...
	setPalette(actpal?QApplication::palette():*WindowManager::ipal);
}
...
void xwindow::get_wmname(void)  // get WM_NAME and ICON_NAME and set caption and border pixmap
{
	...
	XGetWindowProperty(QX11Info::display(), clientid, WindowManager::net_wm_name, 0, 500, FALSE, AnyPropertyType, &type, &format, &nitems, &extra, &data);
	...
	data = NULL;
	XGetWindowProperty(QX11Info::display(), clientid, WindowManager::net_wm_icon_name, 0, 500, FALSE, AnyPropertyType, &type, &format, &nitems, &extra, &data);
	...
	if(wmname != oldwname)
	{
		...
		{
			client = clst.at(i);
			if(client != this && ! (client->get_pflags() & WindowManager::SmallFrame) && wmname == client->ccaption() && client->get_tnumber() >= tnumber)
...
xwindow::~xwindow(void)
{
	...
	if(WindowManager::winf->get_client() == this)
		WindowManager::winf->release_cancel();
	
	tb_pb->remove(this);  // remove from procbar
	WindowManager::cwindows.remove(clientid);
	WindowManager::pwindows.remove(winId());
