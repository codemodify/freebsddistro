...
unsigned long get_atoms(Window w, Atom a, Atom type, unsigned long off, unsigned long *ret, unsigned long nitems, unsigned long *left)
{
    Atom real_type;
    int i, real_format = 0;
    ...
    XGetWindowProperty(dpy, w, a, off, nitems, False, type, &real_type, &real_format, &items_read, &bytes_left, &data);
    ...
    if (real_format == 32 && items_read)
    {
        ...
        if (left) 
		*left = bytes_left / sizeof(long); /* srsly. */
