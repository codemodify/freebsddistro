...
void defaults::read_config(void)
{
		...
		while(! istr.atEnd())
		{
			...
			switch(i)
			{
				case 0:
					...
					if(ival < 10 || ival > 500)
						qapp::logmsg << "ToolbarHeight: Value out of range\n";
					else
						tb_height = ival;
				...
				case 1:
					...
					if(ival < 6 || ival > 500)
						qapp::logmsg << "WindowButtonSize: Value out of range\n";
					else
				...
				case 2:
					...
					if(ival < 1 || ival > 500)
						qapp::logmsg << "LowerBorderHeight: Value out of range\n";
					else
				...
				case 3:
					...
					if(ival < 1 || ival > 500)
						qapp::logmsg << "LowerBorderWidth: Value out of range\n";
					else
				...
				case 4:
					...
					if(ival < 4 && ival > 500)
						qapp::logmsg << "PagerHeight: Value out of range\n";
					else
						pager_height = ival;
				...
				case 6:
					...
					if(ival < 4 || ival > 500)
						qapp::logmsg << "WindowFontHeight: Value out of range\n";
					else
				...
				case 8:
					...
					if(ival < 4 || ival > 500)
						qapp::logmsg << "IconFontHeight: Value out of range\n";
					else
				...
				case 10:
					...
					{
						qapp::logmsg << "VirtualDesktops: Value out of range\n";
						vdesks = 3;
					}
					else
						vdesks = ival;
				...
				case 12:
					...
					if(ival < 0 || ival > 100000)
						qapp::logmsg << "AutoRaiseTime: Value out of range\n";
					else
						autofocustime = ival;
				...
				case 26:
					...
					if(i2 < 3)
					{
						qapp::logmsg << "invalid parameter for maximize\n";
						break;
					}
				...
				case 27:
					...
					if(fval < 1 || fval > 99)
						qapp::logmsg << "TileSplit: Value out of range\n";
					else
						tleftspace = fval/100;
				...
				case 28: 
					...
					if(ival < 0 || ival > 10000)
						qapp::logmsg << "TileMaxOnTab: Value out of range\n";
					else
						maxontab = ival;
				...
				case 29:
					for(i2=0; i2 < 10; i2++)
					{
						...
						if(ival < 1 || ival > 10)
						{
							qapp::logmsg << "TileStart: value out of range\n";
							continue;
						}
				...
				case 30:
					...
					if(ival < 0 || ival > 10000)
						qapp::logmsg << "TileMinframe: Value out of range\n";
					else
						wminframe = ival;
				...
				default:
					qapp::logmsg << "WM: unknown parameter: " << p1 << '\n';
			}
		}
		istr.close();
	}	
	...
	if(info.family() != borderfont.family())
		qapp::logmsg << "WM: no match for font " << borderfont.family() << ", using " << info.family() << " instead\n";

	QFontInfo tinfo(toolbarfont);
	if(tinfo.family() != toolbarfont.family())
		qapp::logmsg << "WM: no match for font " << toolbarfont.family() << ", using " << tinfo.family() << " instead\n";
	...
	if(borderfont.pixelSize() > windowbuttonsize-3)
	{
		...
		qapp::logmsg << "WM: windowborder too small for font, set to " << windowbuttonsize << '\n';
	}
	...
	if(toolbarfont.pixelSize() > tc_height-4)
	{
		...
		qapp::logmsg << "WM: toolbar contents too small for font, set to " << tc_height << '\n';
	}
}
