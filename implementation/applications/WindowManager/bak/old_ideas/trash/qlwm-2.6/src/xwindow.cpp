...
void xwindow::set_title(void)
{
	...
	int twidth = fm.width(*wmn);  // pixel width of text
	int txtstart = 0;

	wtitle.resize(twidth, defaults::windowbuttonsize-3);
	wtitle.fill(qapp::tb->foregroundColor());

	QBitmap mask(wtitle.width(), wtitle.height(), TRUE);  // all transparent
	QPainter painter(&mask);
	painter.setFont(font());

	painter.drawText(txtstart, -1, mask.width()-txtstart, mask.height(), AlignLeft|AlignVCenter, *wmn);
	painter.end();
	wtitle.setMask(mask);
