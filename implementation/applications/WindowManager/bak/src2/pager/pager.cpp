...
void pager::draw_pager(void)   // redraw pager and all clients
{
	...
	// fill active desk
	painter.fillRect(WindowManager::adesk*vw, 0, vw, vh, defaults::pager_active);
	...
	for(i = 0; i < nwins; i++)
	{
		...
		if((client = WindowManager::pwindows[wins[i]]) == NULL || ! client->isVisible() || client->is_unmapped())
			continue;

		// adjust for virtual desktop
		crect.setRect(client->x()+(WindowManager::adesk*dwidth), client->y(), client->width(), client->height());
...
{
	QColor fcol = (crect.x()/dwidth == WindowManager::adesk || crect.right()/dwidth == WindowManager::adesk)?(defaults::pager_visible):(defaults::pager_window);
...
void pager::change_desk(int desk)  // change virtual desktop
{
	...
	if(desk == WindowManager::adesk || desk < 0 || desk >= defaults::vdesks)
		return;

	int dx = (WindowManager::adesk-desk)*dwidth;
	...
	foreach(client, clients)
	{
		...
		if(! (client->get_pflags() & WindowManager::Sticky) && (client->isVisible() || ! curdesk))
		{
